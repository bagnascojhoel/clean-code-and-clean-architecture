...
export default class AppliedCouponRepositoryDatabase implements AppliedCouponRepository {
    ...
    async insert(appliedCoupon: AppliedCoupon): Promise<any> {
        ...
        const insertStatement = `INSERT INTO applied_coupon (coupon_id, order_id) VALUES (?, ?)`
        await this.connection.query(insertStatement, [queryResult.couponId, appliedCoupon.order.code.sequentialId])
    }
}
