...
import DateTimeMother from './objectmothers/DateTimeMother';
...
test('Should throw error when CPF is invalid', () => {
    expect(() => new Order(DOUGLAS_BIRTHDAY, '11122211112', FREIGHT, [])).toThrowError();
})

test('Should throw error when there is no item', () => {
    expect(() => new Order(DOUGLAS_BIRTHDAY, VALID_CPF, FREIGHT, [], COUPON)).toThrowError('Order must have at least one item');
})
...
test('Should create when all parameters are valid', () => {
    ...
    expect(() => new Order(DOUGLAS_BIRTHDAY, VALID_CPF, FREIGHT, [item1, item2, item3], COUPON)).not.toThrow();
})
...
test('Should have price 300 when there is a total of 100 in items and freight is 200', () => {
    ...
    const order = new Order(DOUGLAS_BIRTHDAY, VALID_CPF, freight, [item1, item2]);
...
test('Should have price 80 when total is 110 and there is a coupon for 30%', () => {
    ...
    const freight = Sinon.stub(FREIGHT);
    const order = new Order(DOUGLAS_BIRTHDAY, VALID_CPF, freight, [item1, item2], coupon);
