...
import WarehouseStockEntryOutEventHandler from "../../../../src/application/event-handler/WarehouseStockEntryOutEventHandler"
...
const handler = new WarehouseStockEntryOutEventHandler(repoFactory, eventQueue)

describe('Application > EventHandler > WarehouseStockEntryOutEventHandler', () => {
    beforeEach(async () => {
        Sinon.restore()
        await Promise.all([
    ...
    it('Should persist stock entries', async () => {
        ...
        expect(entries[0].entryType).toEqual("out")
    ...
    it('Should publish stock change event', async () => {
        ...
        expect(capturedArgument.entries[0].entryType).toEqual("out")
